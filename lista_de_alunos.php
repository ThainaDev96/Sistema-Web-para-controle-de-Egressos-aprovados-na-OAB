                                                ...
                                                <div class="card-block">
                                                    <div class="table-responsive dt-responsive">
                                                        <table id="TabelaListaAlunos" class="table table-striped table-bordered nowrap">
                                                            ...
                                                            <tbody>
                                                            <?php //substitui tabela estática pelo do banco com o select
                                                            include 'conexao.php'; // conecta ao banco

                                                            // Seleciona todos os campos da tabela aluno
                                                            $sql = "SELECT * FROM alunos";
                                                            $result = $conn->query($sql);
                                                            //verifica se retornou as linhas
                                                            if ($result->num_rows > 0) {
                                                                while($row = $result->fetch_assoc()) {
                                                                    ...
                                                                    echo '<td>'.$row["telefone"].' / '.$row["celular"].'</td>';

                                                                    // Situação OAB com bolinha colorida
                                                                    if ($row["aprovado_oab"] == "s") {
                                                                        $cor = "green";
                                                                    } else {
                                                                        $cor = "red";
                                                                    }
                                                                    //mostra a bolinha colorida na tabela:
                                                                    echo '<td><span style="display:inline-block; width:16px; height:16px; background-color:'.$cor.'; border-radius:50%;"></span></td>';

                                                                    // Mostra o documento
                                                                    echo '<td>'.$row["doc_oab"].'</td>';


                                                                    // Ações: Editar e Deletar
                                                                    echo '<td>';
                                                                    
                                                                    //echo serve para “imprimir” o HTML com os valores vindo do banco.
                                                                    // Botão Deletar
                                                                    echo '<form style="display:inline;" action="aluno_processa.php" method="post">
                                                                            <input type="hidden" name="id" value="'.$row["id"].'">
                                                                            <button type="submit" name="tipo" value="Deletar" class="btn btn-out btn-danger btn-square">
                                                                                <i class="ti-trash"></i>
                                                                            </button>
                                                                        </form>';

                                                                    //hidden serve para guardar o ID sem mostrar para o usuário.
                                                                    // Botão Editar (abre cadastro com id preenchido)
                                                                    echo '<form style="display:inline;" action="cadastro_de_alunos.php" method="post">
                                                                            <input type="hidden" name="id" value="'.$row["id"].'">
                                                                            <button type="submit"name="tipo" value="Editar" class="btn btn-out btn-primary btn-square">
                                                                                <i class="ti-pencil"></i>
                                                                            </button>
                                                                        </form>';
                                                    ...
                                                    <a href="cadastro_de_alunos.php"class="btn btn-out btn-primary btn-square" style="float: right; margin-right: 16px;min-width: 100px;">
                                                        <i class="ti-plus"></i>Cadastrar
                                                    </a>
                                                </div>
                                            </div> 
